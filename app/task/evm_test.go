...
type evmMock struct {
	...
	logsResp     string // eth_getLogs 响应，空表示返回一条 USDT 转账
	logsRequests []string
}
...
func (m *evmMock) handler(w http.ResponseWriter, r *http.Request) {
	...
	if strings.HasPrefix(strings.TrimSpace(s), "[") {
		...
		items := make([]string, 0)
		for _, req := range gjson.Parse(s).Array() {
			id := req.Get("id").Int()
			...
			items = append(items, fmt.Sprintf(`{"jsonrpc":"2.0","id":%d,"result":{"number":"0x%x","timestamp":"0x%x","transactions":[]}}`, id, id, 1700000000+id))
		}
	...
	log := map[string]any{
		...
		"blockNumber":     fmt.Sprintf("0x%x", testPolFrom+3),
		"transactionHash": testPolTxHash,
		"logIndex":        "0x0",
	}
	resp, _ := json.Marshal(map[string]any{"jsonrpc": "2.0", "id": 1, "result": []any{log}})
	_, _ = w.Write(resp)
}
...
func TestEvmGetLogsFiltersByNetworkContractsAndParsesTransfer(t *testing.T) {
	...
	if len(m.logsRequests) != 1 {
		t.Fatalf("expected one eth_getLogs request, got %d", len(m.logsRequests))
	}
	req := gjson.Parse(m.logsRequests[0])
	...
	tr := transfers[0]
	if tr.TxHash != testPolTxHash || tr.TradeType != model.UsdtPolygon || tr.RecvAddress != testPolRecv || tr.FromAddress != testPolSender || tr.Amount.String() != "4.480096" || tr.BlockNum != testPolFrom+3 {
...
func TestEvmBatchFailuresAreNotSuccessAndRetry(t *testing.T) {
	cases := []struct {
		...
		{"eth_getLogs rpc error", 93363060, &evmMock{logsResp: `{"jsonrpc":"2.0","id":1,"error":{"code":-32005,"message":"query returned more than 10000 results"}}`}},
		{"eth_getLogs result not array", 93363070, &evmMock{logsResp: `{"jsonrpc":"2.0","id":1,"result":null}`}},
	}
