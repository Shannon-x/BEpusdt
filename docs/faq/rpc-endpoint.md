...
| 告警 | 级别 | 触发条件 | 限频 |
|---|---|---|---|
| 区块扫描放弃 | 收款 | 区块重试 10 次仍失败 | 5 分钟 |
| 扫描任务重试耗尽 | 收款 | 任务自动重试 20 次仍失败 | 每任务 1 次 |
| 扫描区间跳过 | 运维提示 | 重启 / 链头跳跃导致区间被跳过 | 10 分钟 |
| 区块扫描停滞 | 收款 | 有待支付订单但 2 分钟内没有成功扫描 | 10 分钟 |
| 扫描队列拥堵 | 运维提示 | 实时队列达到 100 个区块 | 10 分钟 |
| 商户回调积压 | 收款 | 最早待发送回调等待超过 30 分钟 | 10 分钟 |
| 订单回调重试耗尽 | 收款 | 单个订单回调达到 `notify_max_retry` 次 | 每订单 1 次 |
| 对账补认单 | 运维提示 | 对账任务为此前未匹配的入账补认了订单 | 10 分钟 |

---

## 扫块追不上出块速度怎么办

收到「扫描队列拥堵」「扫描区间跳过」通常不是节点坏了，而是**解析速度低于出块速度**：队列涨满后停止入队，链头继续前进，超过 `block_height_max_diff` 就会记一条 gap 任务并对齐链头，于是反复循环。

各链出块速度与默认并发（按单次请求 1~2 秒估算）：

| 链 | 出块速度 | 默认并发 | 说明 |
|---|---|---|---|
| Solana | ~2.5 slot/s | 10 | 最容易追不上，v1.26.5 起默认由 3 提高到 10 |
| Polygon / BSC / Base / Arbitrum 等 | 0.25~2 块/s | 3 | 按 `block_batch_size` 批量取，实际请求数更低 |
| Tron | ~0.33 块/s | 2 | 足够 |
| Aptos | 按版本区间 | 3 | 足够 |

处理顺序：

1. 确认该链配了**多个不同服务商的节点**（逗号分隔），单节点限流时会自动切换；
2. 调高 `block_scan_workers`（后台「基础设置 → 扫块并发数」），Solana 建议 10~16；
3. 若调高后频繁出现 429 / 限流，说明节点本身扛不住，应换更快的节点而不是继续加并发；
4. 被跳过的区间不会直接导致漏单——待支付订单由订单回溯覆盖；需要完整补扫可用 `bepusdt scan replay` 或 `POST /api/scan/replay` 分段回放。
